    ...
    static bool IsContinuationLine(Sci_PositionU szLine, Accessor &styler) {
        ...
        while (startPos < endPos) {
            ...
            if (!(stylech == SCE_POWERPRO_COMMENTBLOCK)) {
                ...
                if (ch > 0 && chPrev > 0 && chPrevPrev > 0 && !isspacechar(ch) && !isspacechar(chPrev) &&
...
//		currentPos will be the position of 'a'
    ...
    static void
    ColourisePowerProDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *keywordlists[],
        ...
        for (; sc.More(); sc.Forward()) {
            ...
            switch (sc.state) {
                ...
                case SCE_POWERPRO_IDENTIFIER:
                    ...
                    if ((sc.ch > 0) && !setWord.Contains(
                            sc.ch)) {  // || (sc.ch == '.')) { // use this line if you want to match keywords with a . ie: win.debug will only match win.debug neither win nor debug will be colorized separately
                        char s[1000];
                        if (caseSensitive) {
            ...
            if (sc.state == SCE_POWERPRO_DEFAULT) {
                if (sc.Match('?', '\"')) {
                    ...
                    sc.SetState(SCE_POWERPRO_COMMENTLINE);
                           ...
                           sc.ch == ';') {        //legacy comment that can only have blank space in front of it
                    sc.SetState(SCE_POWERPRO_COMMENTLINE);
                } else if (sc.Match(";;")) {
                    sc.SetState(SCE_POWERPRO_COMMENTLINE);
                } else if (sc.ch == '\"') {
    ...
    static void
    ColourisePowerProDocWrapper(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *keywordlists[],
                                Accessor &styler) {
