...
#define YAML_STATE_TEXT            (5 << YAML_STATE_BITSIZE)
    ...
    static void ColouriseYAMLLine(
        ...
        while (i < lengthLine) {
            if (lineBuffer[i] == '\'' || lineBuffer[i] == '\"') {
                ...
                if (KeywordAtChar(&lineBuffer[i], &lineBuffer[startComment],
                                  keywords)) { // Convertible value (true/false, etc.)
    ...
    static void
    ColouriseYAMLDoc(Sci_PositionU startPos, Sci_Position length, int, WordList *keywordLists[], Accessor &styler) {
        ...
        if (linePos > 0) {    // Last line does not have ending characters
            ColouriseYAMLLine(lineBuffer, lineCurrent, linePos, startLine, startPos + length - 1, *keywordLists[0],
                              styler);
        }
    }
