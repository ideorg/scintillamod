...
*/
    ...
    inline bool HandleWord(Sci_PositionU &cur, Sci_PositionU one_too_much, Accessor &styler, WordList *keywordlists[]) {
        ...
        for (;;) {
            ch = styler.SafeGetCharAt(cur);
            if ((ch != '_') && (ch != '-') &&
                !(IsASCII(ch) && (islower(ch) || isupper(ch) || isdigit(ch))))
                break;
    ...
    static void ColouriseOpalDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *keywordlists[],
                                 Accessor &styler) {
