...
namespace ScintillaMod {
    ...
    static void ColouriseRebolDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *keywordlists[],
                                  Accessor &styler) {
        ...
        for (; sc.More(); sc.Forward()) {
            ...
            if (sc.state == SCE_REBOL_IDENTIFIER || (sc.state >= SCE_REBOL_WORD && sc.state <= SCE_REBOL_WORD8)) {
                ...
                switch (sc.ch) {
                    case 'x':
                        ...
                        break;
                    case ':':
                        sc.ChangeState(SCE_REBOL_TIME);
                        break;
                    case '-':
                    case '/':
                        sc.ChangeState(SCE_REBOL_DATE);
                        break;
                    case '.':
                        if (++dotCount >= 2) sc.ChangeState(SCE_REBOL_TUPLE);
                        break;
                }
            }
            ...
            if (sc.state == SCE_REBOL_DEFAULT) {
                ...
                } else if (IsADigit(sc.ch) || sc.ch == '+' || sc.ch == '-' || /*Decimal*/ sc.ch == '.' ||
                           sc.ch == ',') {
                    dotCount = 0;
