...
namespace ScintillaMod {
    ...
    static void ColourTo(Accessor &styler, Sci_PositionU end, unsigned int attr, bool bInAsm) {
        if ((bInAsm) &&
            (attr == SCE_C_OPERATOR || attr == SCE_C_NUMBER || attr == SCE_C_DEFAULT || attr == SCE_C_WORD ||
             attr == SCE_C_IDENTIFIER)) {
...
// returns 1 if the item starts a class definition, and -1 if the word is "end", and 2 if the word is "asm"
    static int classifyWordTAL(Sci_PositionU start, Sci_PositionU end, /*WordList &keywords*/WordList *keywordlists[],
                               Accessor &styler, bool bInAsm) {
        int ret = 0;
    ...
    static void FoldTALDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *[],
        ...
        for (Sci_PositionU i = startPos; i < endPos; i++) {
            ...
            if (stylePrev == SCE_C_DEFAULT &&
                (style == SCE_C_WORD || style == SCE_C_UUID || style == SCE_C_PREPROCESSOR)) {
