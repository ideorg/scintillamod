...
namespace ScintillaMod {
    ...
    struct PPDefinition {
        ...
        PPDefinition(Sci_Position line_, const std::string &key_, const std::string &value_, bool isUndef_ = false,
...
// Options used for LexerVerilog
    ...
    class LexerVerilog : public DefaultLexer {
        ...
        enum {
            foldExternFlag = 0x01, foldWaitDisableFlag = 0x02, typedefFlag = 0x04, protectedFlag = 0x08
        };
        ...
        for (Sci_PositionU i = startPos; i < endPos; i++) {
            ...
            if (ch == '`') {
                ...
                if (styler.Match(j, "protected")) {
                    ...
                    stateCurrent &= ~protectedFlag;
                    levelNext--;
                } else if (!(stateCurrent & protectedFlag) && options.foldPreprocessor &&
                           (style == SCE_V_PREPROCESSOR)) {
